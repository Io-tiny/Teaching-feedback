...
import teacherForm from "@/components/administrator/teacherForm";
export default {
  data() {
    return {
      Datas: [],
  ...
  methods: {
    ...
    getData: function() {
      ...
      this.pagesize = 5;
      this.$http
        .post("http://localhost:3000/api/administrator/getTeacherData", {
            sort: 'teacher'
        })
    ...
    delData: function(index, row) {
      self = this;
      this.$http
        .post("http://localhost:3000/api/administrator/delTeacherData", {
          id: row.id
        })
        .then(function(response) {
          self.getData();
    ...
    saveData: function(Data) {
      ...
      if (this.Data.id) {
        this.$http
          .post("http://localhost:3000/api/administrator/updateTeacherData", {
            id: self.Data.id,
            name: self.Data.name,
    ...
    addData: function() {
      ...
      this.$http
        .post("http://localhost:3000/api/administrator/addTeacherData", {
          name: self.Data.name,
          class: self.Data.class,
          username: self.Data.username,
          password: self.Data.password,
    ...
    searchData: function() {    
      self = this;
      this.$http
        .post("http://localhost:3000/api/administrator/searchTeacherData", {
          name: "%" + self.inputData + "%",
          sort:'teacher'
        })
        .then(function(response) {
